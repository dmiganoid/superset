...
// Функция-обработчик переключения тумблера
// Замени функцию onMaintenanceChange на эту:
  const onMaintenanceChange = (checked: boolean) => {
    // 1. Берем текущий JSON из стейта
    ...
    // 2. Обновляем наши поля
    metaObj['maintenance_mode'] = checked;
    
    if (checked && !metaObj['maintenance_message']) {
       metaObj['maintenance_message'] = "Дашборд на техническом обслуживании";
    }

    // 3. Превращаем в строку (используем библиотечный stringify для надежности)
    let newJsonString = '';
    try {
    ...
    // 4. ВАЖНО: Обновляем и стейт (откуда берет onFinish), и форму
    setJsonMetadata(newJsonString); // <--- ЭТОГО НЕ БЫЛО
    form.setFieldsValue({
      json_metadata: newJsonString,
    });
  };

  // Вспомогательная функция для получения текущего статуса (для defaultChecked)
  const getIsMaintenance = () => {
     try {
       // Читаем напрямую из State, чтобы переключатель реагировал мгновенно
       const meta = jsonMetadata ? JSON.parse(jsonMetadata) : {};
       return !!meta.maintenance_mode;
     } catch {
       return false;
     }
  };
    ...
    >
      ...
      >
        ...
        <Collapse
          ...
          modalMode
          items={[
            // --- ВСТАВИТЬ ОТСЮДА (Секция Статус) ---
            {
              key: 'status',
              label: (
                <CollapseLabelInModal
                  title={t('Status')}
                  subtitle={t('Maintenance mode controls')}
