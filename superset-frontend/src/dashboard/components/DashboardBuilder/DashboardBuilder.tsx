...
const DashboardBuilder = () => {
  ...
  const maintenanceData = useMemo(() => {
    ...
    // Если это объект (как сейчас у нас сделано правильно)
    if (metadata.maintenance_mode) {
      return {
        isMaintenance: true,
        message: metadata.maintenance_message || "Технические работы"
      };
    }
    return null;
  }, [dashboardInfo]);
    ...
    <DashboardWrapper>
      ...
      <StyledContent fullSizeChartId={fullSizeChartId}>
        ...
        >
          ...
          >
            {showDashboard ? (
              maintenanceData?.isMaintenance && !editMode ? (
                <div
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <Result
                    ...
                    subTitle={maintenanceData.message}
                    extra={
                      canEdit ? (
                        <div style={{ color: '#999', fontSize: '12px' }}>
                          Если Вы видите это сообщение, то можете нажать <b>Edit</b>, чтобы отключить режим.
                        </div>
                      ) : null
                    }
                  />
                ...
                // 2. Если режима обслуживания нет, идем по стандартному пути
      ...
      {/* --- НАЧАЛО ВСТАВКИ: Кнопка выхода (Классический стиль + Иконка) --- */}
      {standaloneMode === DashboardStandaloneMode.Report && (
        <div
          style={{
            position: 'fixed',
            bottom: '15px',
        ...
        >
          <Button
            ...
            css={css`
              /* Небольшая тень, чтобы кнопка не сливалась с фоном */
            ...
            `}
          >
            {/* SVG Иконка "Сжатие/Выход" */}
            <svg 
              width="24" 
              height="24" 
              viewBox="0 0 24 24" 
              fill="currentColor" /* Наследует белый цвет текста кнопки */
